...
#include "ana6705_a51_param.h"
#include "dd.h"

#if defined(CONFIG_EXYNOS_DECON_MDNIE)
#include "mdnie.h"
#include "mdnie_table_a51.h"
#endif
...
static int ana6705_read_id(struct lcd_info *lcd)
{
...
static int ana6705_read_coordinate(struct lcd_info *lcd)
{
	int ret = 0;
...
static int ana6705_read_manufacture_info(struct lcd_info *lcd)
{
	int ret = 0;
...
#if 0
static int ana6705_read_chip_id(struct lcd_info *lcd)
{
	int ret = 0;
...
#endif
static int ana6705_read_elvss(struct lcd_info *lcd)
{
	int ret = 0;
...
static int ana6705_read_rdnumpe(struct lcd_info *lcd)
{
	int ret = 0;
...
static int ana6705_read_esderr(struct lcd_info *lcd)
{
	int ret = 0;
...
static int ana6705_read_rddsdr(struct lcd_info *lcd)
{
	int ret = 0;
...
static int ana6705_read_rddpm(struct lcd_info *lcd)
{
	int ret = 0;
...
static int ana6705_read_rddsm(struct lcd_info *lcd)
{
	int ret = 0;
...
unsigned int get_panel_bigdata(struct dsim_device *dsim)
{
	...
	ana6705_read_rddpm(lcd);
	ana6705_read_rddsm(lcd);
...
static int ana6705_read_init_info(struct lcd_info *lcd)
{
	int ret = 0;
	...
	ana6705_read_coordinate(lcd);
	ana6705_read_manufacture_info(lcd);
//	ana6705_read_chip_id(lcd);
	ana6705_read_elvss(lcd);
...
static int ana6705_exit(struct lcd_info *lcd)
{
	int ret = 0;
	...
	ana6705_read_rdnumpe(lcd);

	DSI_WRITE(SEQ_TEST_KEY_ON_F0, ARRAY_SIZE(SEQ_TEST_KEY_ON_F0));
	ana6705_read_esderr(lcd);
	DSI_WRITE(SEQ_TEST_KEY_OFF_F0, ARRAY_SIZE(SEQ_TEST_KEY_OFF_F0));
#endif
...
static int ana6705_displayon(struct lcd_info *lcd)
{
	int ret = 0;
...
static int ana6705_init(struct lcd_info *lcd)
{
	int ret = 0;
			...
			unsigned long event, void *data)
{
	...
	if (fb_blank == FB_BLANK_UNBLANK)
		ana6705_displayon(lcd);

	return NOTIFY_DONE;
}

static int ana6705_register_notifier(struct lcd_info *lcd)
{
...
static int ana6705_probe(struct lcd_info *lcd)
{
	int ret = 0;
	...
	ret = ana6705_read_init_info(lcd);
	if (ret < 0)
...
#if defined(CONFIG_EXYNOS_DOZE)
static int ana6705_setalpm(struct lcd_info *lcd, int mode)
{
	int ret = 0;
	...
	case ALPM_ON_HIGH:
		DSI_WRITE(SEQ_TEST_KEY_ON_F0, ARRAY_SIZE(SEQ_TEST_KEY_ON_F0));
		DSI_WRITE(SEQ_HLPM_AOR_GPARA, ARRAY_SIZE(SEQ_HLPM_AOR_GPARA));
...
static int ana6705_enteralpm(struct lcd_info *lcd)
{
	int ret = 0;
	...
	ret = ana6705_setalpm(lcd, lcd->alpm);
	if (ret < 0)
...
static int ana6705_exitalpm(struct lcd_info *lcd)
{
	int ret = 0;
